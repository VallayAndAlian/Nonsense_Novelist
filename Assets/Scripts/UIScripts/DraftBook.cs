...
/// 挂在草稿本母节点上。
/// </summary>
public class DraftBook : BasePanel
{
    // 不换行的的空格符
    ...
    public void closeDraft()
    {
        ...
        for (int i = parent.childCount-1; i >=0; i--)
        {
            ...
            {
                DraftMgr.instance.content.RemoveAt(parent.GetChild(i).GetComponent<DragDraftText>().index);
    ...
    void InitDraft()
    {
        ...
        //生成句子,绑定组件
        for (int i = 0; i < DraftMgr.instance.content.Count; i++)
        {
            PoolMgr.GetInstance().GetObj(sentenseObj, (obj) =>
            {
                ...
                _showText.text = DraftMgr.instance.content[i];
                ...
                //转行            
                var count = Mathf.Floor((sizeFont * (DraftMgr.instance.content[i].Length))/ (sizeWidth - _showText.margin.x));
    ...
    public void CloseEditText(TMP_InputField _inputField)
    {
        ...
        //修改库里的文本
        DraftMgr.instance.content[_inputField.GetComponent<DragDraftText>().index] = _inputField.text;
